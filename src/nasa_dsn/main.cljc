(ns nasa_dsn.main
  "Kotodama WASM entrypoint for the Nasa_dsn clean-room actor (L4) — Clojure port.
  ...
  facts (`nasa_dsn.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "nasa_dsn")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"nasa_dsn.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "nasa_dsn-compat" :tier tier :entities entities} 200])
...
(defn start! [] :nasa_dsn-compat/ready)
