(ns nasa_dsn.main-test
  "Contract + behavioral test for the nasa_dsn-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "nasa_dsn-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'nasa_dsn.main-test)]
